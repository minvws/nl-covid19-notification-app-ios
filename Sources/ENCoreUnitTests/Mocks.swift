...
class NetworkControllingMock: NetworkControlling {
    ...
    var postKeysHandler: (([DiagnosisKey], LabConfirmationKey) -> (AnyPublisher<(), NetworkError>))?
    func postKeys(keys: [DiagnosisKey], labConfirmationKey: LabConfirmationKey) -> AnyPublisher<(), NetworkError> {
        ...
        if let postKeysHandler = postKeysHandler {
            return postKeysHandler(keys, labConfirmationKey)
        }
...
class CryptoUtilityMock: CryptoUtility {
    ...
    func validate(data: Data, signature: Data) -> Bool {
        ...
        if let validateHandler = validateHandler {
            return validateHandler(data, signature)
        }
        return false
    }
...
class ThankYouBuildableMock: ThankYouBuildable {
    ...
    var buildCallCount = 0
    var buildHandler: ((ThankYouListener) -> (ViewControllable))?
...
class ExposureDataControllingMock: ExposureDataControlling {
    ...
    var scheduleOperationsCallCount = 0
    var scheduleOperationsHandler: (() -> ())?
    func scheduleOperations() {
        scheduleOperationsCallCount += 1
        if let scheduleOperationsHandler = scheduleOperationsHandler {
            scheduleOperationsHandler()
        }
    }
...
class ExposureDataOperationProviderMock: ExposureDataOperationProvider {
    ...
    var uploadDiagnosisKeysOperationHandler: (([DiagnosisKey], LabConfirmationKey) -> (UploadDiagnosisKeysDataOperation))?
    func uploadDiagnosisKeysOperation(diagnosisKeys: [DiagnosisKey], labConfirmationKey: LabConfirmationKey) -> UploadDiagnosisKeysDataOperation {
        uploadDiagnosisKeysOperationCallCount += 1
        if let uploadDiagnosisKeysOperationHandler = uploadDiagnosisKeysOperationHandler {
            return uploadDiagnosisKeysOperationHandler(diagnosisKeys, labConfirmationKey)
        }
...
class NetworkManagingMock: NetworkManaging {
    ...
    var getDiagnosisKeysCallCount = 0
    var getDiagnosisKeysHandler: ((String, @escaping (Result<[URL], NetworkError>) -> ()) -> ())?
